...
def load_model(model_name: str | Path, max_length: int):
...
def _user_prompt(text: str) -> str:
    return f"Translate the following statement in natural language to Lean:\n{text}"


def _formal_prompt(natural_language: str, formal: str, eos_token: str | None) -> str:
    eos = eos_token or ""
    return (
        "Translate the following statement in natural language to Lean:\n"
        f" {natural_language}\nLean statement:\n {formal}{eos}"
    )


def _get_chat_marker(tokenizer: PreTrainedTokenizer) -> list[int]:
...
def _find_nl_end_index(prompt_ids: list[int], marker: list[int]) -> int:
    marker_length = len(marker)
    for idx in range(len(prompt_ids) - marker_length + 1):
        if prompt_ids[idx : idx + marker_length] == marker:
            return idx - 1
    raise ValueError("Completion marker not found in prompt.")


def score_example(nl: str, fl: str, model, tokenizer):
    collator = CustomCollator(tokenizer=tokenizer, mask_inputs=False, mlm=False)
    chat_marker = _get_chat_marker(tokenizer)
...
def compute_formal_align_score(inputs: dict, model_outputs: CausalLMOutput):
    ...
    # -1 to zero index, then -1 to get := token
    fl_index = torch.sum(inputs["attention_mask"], dim=1) - 1 - 1
...
def tokenize_chat(examples, marker: list[int], tokenizer: PreTrainedTokenizer):
    ...
    nl_end_idx = [_find_nl_end_index(prompt, marker) for prompt in prompts]
    batch = {
        "input_ids": prompts,
        "attention_mask": [[1] * len(prompt) for prompt in prompts],
        "nl_end_idx": nl_end_idx,
        "fl_start_idx": [idx + len(marker) + 2 for idx in nl_end_idx],
    }
...
def tokenize(examples, marker: list[int], tokenizer: PreTrainedTokenizer):
    ...
    nl_end_idx = [_find_nl_end_index(prompt, marker) for prompt in input_ids]
    batch = dict(encoded)
    batch["nl_end_idx"] = nl_end_idx
    batch["fl_start_idx"] = [idx + len(marker) + 2 for idx in nl_end_idx]
    batch["text"] = prompts
    if "label" in examples:
        batch["aligned"] = examples["label"]
    return batch
...
def train(
    ...
    negative_sample_ratio: Annotated[
        float,
        Option(
            help="% of positive dataset to sample as negative examples",
    ...
    mask_inputs: Annotated[
        ...
        Option(
            "--mask-inputs",
            help="train on completions only",
            rich_help_panel="Training Config",
        ),
    ] = False,
    # fmt:on
...
def predict_herald(
    ...
    hf_dataset = hf_dataset.map(
        lambda batch: tokenize_chat(batch, chat_marker, tokenizer),
        batched=True,
