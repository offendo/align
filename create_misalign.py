...
def modify_constant(expression):
    ...
    if len(theorem_parts) < 2:
        print("not in format theorem xx : ```", expression, '```')
    ...
    if constants:
        ...
        theorem_body = re.sub(r"(?<!\\u)\b" + re.escape(chosen_constant) + r"\b", new_constant, theorem_body, count=1)
...
def introduce_variable(expression):
    ...
    # extract the part before ':=', if there is such a part
    ...
    modified_expression = f"{theorem_parts[0]}:{theorem_body}" + (":=" + parts[1] if len(parts) > 1 else "")
...
def modify_batch(batch, seed, all_theorems):
    random.seed(seed)
    modified_data = []
    ...
    total_samples = len(batch['input'])
    modifications_per_theorem = [
    ...
    modified_samples = {'input': [], 'output': [], 'label': [], 'misalign_type': []}
    for i, (nat_theorem, form_theorem) in enumerate(zip(batch['input'], batch['output'])):
        all_responses = all_theorems["output"]

        original_response = form_theorem
        modified_responses = set()
        ...
        for j, chosen_modification_type in enumerate(modifications_per_theorem[i]):
            ...
            modified_samples['input'].append(nat_theorem)
            modified_samples['output'].append(new_response)
            modified_samples['label'].append(0)
            modified_samples['misalign_type'].append(modifications_per_theorem[i][j])

            modified_responses.add(new_response)
    return modified_samples


def modify_dataset_hf(dataset, seed):
    modified = dataset.map(lambda batch: modify_batch(batch, seed, dataset), batched=True, num_procs=8)
...
def modify_dataset(data, seed):
    ...
    for i, sample in enumerate(tqdm(data)):
        ...
        for j, chosen_modification_type in enumerate(modifications_per_theorem[i]):
            ...
            modified_sample["outputs"].append(
                {"response": new_response, "label": False, "misalign_type": modifications_per_theorem[i][j]}
            )
    ...
    avg_coverage_rate = {
        strategy: count / (num_modifications * total_samples) for strategy, count in strategy_counts.items()
    }
...
def main(input_file, output_path, seed):
    ...
    if input_file.endswith(".json"):
        ...
        data = load_dataset(input_file, split='train')
        modified_data = modify_dataset_hf(data, seed)
        df = modified_data.to_pandas()
